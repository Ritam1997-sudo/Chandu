...
// Automatic Resize to 100KB
function resizeTo100KB() {
  if (!originalImage) {
  ...
  const maxWidth = 400; // Adjusted for 100KB
...
// Custom Resize to 100KB
function customResizeTo100KB() {
  if (!originalImage) {
    alert('Please upload an image first!');
    return;
  }
...
autoResizeBtn.addEventListener('click', resizeTo100KB);
customResizeBtn.addEventListener('click', customResizeTo100KB);
downloadBtn.addEventListener('click', () => {
  if (!resizedImageBlob) return;
  const link = document.createElement('a');
  link.href = URL.createObjectURL(resizedImageBlob);
  link.download = `resized-image-100kb.jpg`;
  link.click();
});
